...
const CMSPage = {
    ...
    // Architecture:
    //   pane
    //     ├─ #lms-header   (title + New Course btn)
    //     ├─ #lms-overlay  (inline form panel — persists across tree re-renders)
    //     └─ #lms-tree     (course accordion — safe to innerHTML swap)
    //
    // Rules:
    //   • Only #lms-tree gets innerHTML-replaced on refresh
    //   • #lms-overlay is NEVER destroyed by a tree refresh
    //   • All async calls are properly awaited
    //   • No prompt() — all forms are inline HTML panels

    async _lms() {
        const pane = document.getElementById('cms-pane-lms');
        if (!pane) return;

        // Build skeleton once; subsequent calls only refresh the tree
        if (!document.getElementById('lms-tree')) {
            ...
            <div id="lms-header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
                ...
                <button class="cb cb-p" onclick="CMSPage._lmsShowCourseForm()">
    ...
    async _lmsRefreshTree() {
        ...
        if (!courses.length) {
            ...
            <div class="text-center py-12 text-gray-500 text-sm">
                <i class="fas fa-graduation-cap text-3xl block mb-3 opacity-20"></i>
                No courses yet — click <strong class="text-purple-400">New Course</strong> to get started.
            </div>`;
            return;
        }
        ...
        <div class="lms-module" id="lms-course-${c.id}">
            <div class="lms-module-header" onclick="CMSPage._lmsToggle(${c.id})">
                <i class="fas fa-chevron-right text-gray-600 text-xs" id="lms-arr-${c.id}"
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="text-gray-500 text-xs">${this._e(c.level || 'Beginner')} &bull;
                        ${c.module_count || 0} modules &bull; ${c.lesson_count || 0} lessons &bull;
                        ${c.price > 0 ? '$' + Number(c.price).toFixed(2) : 'Free'}
                    </div>
                </div>
                ${this._pub(c.is_published)}
                <div style="display:flex;gap:.3rem;flex-shrink:0;" onclick="event.stopPropagation()">
                    <button class="cb cb-g" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsShowCourseForm(${c.id})" title="Edit course">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="cb ${c.is_published ? 'cb-y' : 'cb-gr'}" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsTogglePub(${c.id})">
                        ${c.is_published ? 'Unpublish' : 'Publish'}
                    </button>
                    <button class="cb cb-r" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsDeleteCourse(${c.id})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            <div id="lms-body-${c.id}" style="display:none;">
                <div style="padding:.65rem 1rem;background:#0d1117;display:flex;justify-content:space-between;
                    ...
                    <button class="cb cb-p" style="font-size:.7rem;padding:.25rem .75rem;"
                            onclick="CMSPage._lmsShowModuleForm(${c.id})">
                        <i class="fas fa-plus"></i> Add Module
                    </button>
                </div>
                <div id="lms-modules-${c.id}">
                    <div class="text-gray-600 text-xs text-center py-3">
    ...
    async _lmsShowCourseForm(id) {
        id = parseInt(id) || null;
        let d = { title:'', description:'', level:'Beginner', price:0,
                  thumbnail:'', preview_video:'', is_published:false,
                  certificate_enabled:false, pass_percentage:70 };
        if (id) {
            try {
                const list = await API.cms.listCourses();
                d = list.find(c => c.id === id) || d;
            } catch(_) {}
        }
        this._editingId = id;
        this._lmsOverlay(`
        <div class="chdr">
            ...
            <button class="cb cb-g" onclick="CMSPage._lmsCloseOverlay()"><i class="fas fa-times"></i></button>
        </div>
        <div class="crow crow2">
            <div><label class="cl">Title *</label>
                <input class="ci" id="cf-title" value="${this._e(d.title)}" placeholder="Course title"></div>
            <div><label class="cl">Level</label>
                <select class="cs" id="cf-level">
                    ${['Beginner','Intermediate','Advanced'].map(l =>
                        `<option${d.level === l ? ' selected' : ''}>${l}</option>`).join('')}
                </select></div>
        </div>
        <div class="crow"><div><label class="cl">Description</label>
            <textarea class="cta" id="cf-desc" style="min-height:70px;">${this._e(d.description)}</textarea>
        </div></div>
        <div class="crow crow2">
            <div><label class="cl">Price (0 = Free)</label>
                <input type="number" class="ci" id="cf-price" step="0.01" min="0" value="${d.price || 0}"></div>
            <div><label class="cl">Pass % for Certificate</label>
                <input type="number" class="ci" id="cf-pass" min="0" max="100" value="${d.pass_percentage || 70}"></div>
        </div>
        <div class="crow crow2">
            ...
            <div><label class="cl">Preview Video URL</label>
                <input class="ci" id="cf-preview" value="${this._e(d.preview_video)}" placeholder="YouTube/Vimeo URL"></div>
        </div>
        <div style="display:flex;gap:1.5rem;margin-top:.5rem;align-items:center;flex-wrap:wrap;">
            ...
            <button class="cb cb-p" onclick="CMSPage._lmsSaveCourse()">
                <i class="fas fa-save"></i> ${id ? 'Update' : 'Create'} Course
            </button>
        </div>`);
    },
    ...
    async _lmsDeleteCourse(id) {
        if (!confirm('Delete course and ALL its modules, lessons and quizzes?')) return;
        try {
    ...
    async _lmsLoadModules(cid) {
        const el = document.getElementById(`lms-modules-${cid}`);
        if (!el) return;
        let mods = [];
        try { mods = await API.cms.listModules(cid); } catch (e) {
            el.innerHTML = `<div class="text-red-400 text-xs text-center py-3">${e.message}</div>`;
            return;
        }
        if (!mods.length) {
            el.innerHTML = `
            <div style="padding:1rem 1.5rem;text-align:center;">
                ...
                <button class="cb cb-p" style="font-size:.72rem;"
                        onclick="CMSPage._lmsShowModuleForm(${cid})">
            ...
            <div style="display:flex;align-items:center;gap:.75rem;padding:.6rem 1rem .6rem 2rem;background:#161f2e;">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <span class="text-gray-600 text-xs ml-2">
                        ${m.lesson_count || 0} lessons &bull; ${m.quiz_count ? 'Has quiz' : 'No quiz'}
                    </span>
                </div>
                <div style="display:flex;gap:.3rem;flex-shrink:0;">
                    <button class="cb cb-g" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsShowModuleForm(${cid}, ${m.id}, '${this._e(m.title).replace(/'/g,"\\'")}', '${this._e(m.description || '').replace(/'/g,"\\'")}', ${m.order_index || 0})">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="cb cb-p" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsShowLessonForm(${cid}, ${m.id})">
                        <i class="fas fa-plus"></i> Lesson
                    </button>
                    <button class="cb cb-y" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsShowQuizPanel(${m.id})">
                        <i class="fas fa-question-circle"></i> Quiz
                    </button>
                    <button class="cb cb-r" style="font-size:.7rem;padding:.25rem .6rem;"
                            onclick="CMSPage._lmsDeleteModule(${m.id}, ${cid})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            <div id="lms-lessons-${m.id}">
                <div class="text-gray-700 text-xs text-center py-2">
    ...
    _lmsShowModuleForm(cid, mid, title, desc, order) {
        ...
        <div class="crow crow2">
            <div><label class="cl">Module Title *</label>
                <input class="ci" id="mf-title" value="${this._e(title)}" placeholder="e.g. Introduction to Forex"></div>
            <div><label class="cl">Order Index</label>
                <input type="number" class="ci" id="mf-order" value="${order}" min="0"></div>
        </div>
        <div class="crow"><div><label class="cl">Description (optional)</label>
            <textarea class="cta" id="mf-desc" style="min-height:60px;">${this._e(desc)}</textarea>
        </div></div>
        <div style="display:flex;gap:.75rem;margin-top:.25rem;">
            <button class="cb cb-p" onclick="CMSPage._lmsSaveModule(${cid}, ${mid || 'null'})">
                <i class="fas fa-save"></i> ${mid ? 'Update' : 'Add'} Module
            </button>
    ...
    async _lmsSaveModule(cid, mid) {
        ...
        try {
            if (mid) {
                await API.cms.updateModule(mid, p);
                this._toast('Module updated');
            } else {
                await API.cms.createModule(p);
                this._toast('Module added');
            }
            this._lmsCloseOverlay();
            await this._lmsLoadModules(cid);
        } catch (e) {
    ...
    async _lmsDeleteModule(mid, cid) {
        if (!confirm('Delete module and all its lessons and quiz?')) return;
        try {
            await API.cms.deleteModule(mid);
            this._toast('Module deleted');
            this._lmsCloseOverlay();
            await this._lmsLoadModules(cid);
        } catch (e) { this._toast(e.message, 'error'); }
    },
    ...
    async _lmsLoadLessons(mid) {
        ...
        <div class="lms-lesson">
            ...
            <span class="text-gray-300 text-xs flex-1">
                ${String(l.order_index || 0).padStart(2,'0')}. ${this._e(l.title)}
            </span>
            ...
            <div style="display:flex;gap:.25rem;">
                <button class="cb cb-g" style="font-size:.68rem;padding:.2rem .5rem;"
                        onclick="CMSPage._lmsShowLessonForm(null, ${mid}, ${JSON.stringify(l).replace(/"/g,'&quot;')})">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="cb cb-r" style="font-size:.68rem;padding:.2rem .5rem;"
                        onclick="CMSPage._lmsDeleteLesson(${l.id}, ${mid})">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
        </div>`).join('');
    ...
    _lmsShowLessonForm(cid, mid, lesson) {
        ...
        this._editingId = isEdit ? lesson.id : null;
        const d = lesson || { title:'', content:'', video_url:'', attachment_url:'',
                               duration_minutes:0, order_index:0, is_free_preview:false,
                               course_id: cid };
        this._lmsOverlay(`
        <div class="chdr">
            <h4 class="text-white font-semibold">
        ...
        <div class="crow crow2">
            <div><label class="cl">Title *</label>
                <input class="ci" id="lf-title" value="${this._e(d.title)}" placeholder="Lesson title"></div>
            <div><label class="cl">Video URL</label>
                <div style="display:flex;gap:.4rem;">
                    <input class="ci" id="lf-video" value="${this._e(d.video_url)}" placeholder="YouTube/Vimeo…">
                    <button class="cb cb-g" onclick="CMSPage._pickMedia('lf-video','video')">
        ...
        <div class="crow crow2">
            <div><label class="cl">Attachment URL</label>
                <div style="display:flex;gap:.4rem;">
                    ...
                    <button class="cb cb-g" onclick="CMSPage._pickMedia('lf-attach','docs')">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;">
                <div><label class="cl">Duration (min)</label>
                    <input type="number" class="ci" id="lf-dur" value="${d.duration_minutes || 0}" min="0"></div>
                <div><label class="cl">Order</label>
                    <input type="number" class="ci" id="lf-ord" value="${d.order_index || 0}" min="0"></div>
            </div>
        </div>
        <div class="crow"><div><label class="cl">Content / Notes</label>
            <textarea class="cta" id="lf-content" style="min-height:100px;">${this._e(d.content)}</textarea>
        </div></div>
        <div style="display:flex;gap:1.5rem;align-items:center;margin-top:.5rem;flex-wrap:wrap;">
            ...
            <button class="cb cb-p" onclick="CMSPage._lmsSaveLesson(${mid}, ${cid || d.course_id || 0})">
                <i class="fas fa-save"></i> ${isEdit ? 'Update' : 'Add'} Lesson
            </button>
            <button class="cb cb-g" onclick="CMSPage._lmsCloseOverlay()">Cancel</button>
        </div>`);
    },
    ...
    async _lmsSaveLesson(mid, cid) {
        ...
        const p = {
            ...
            attachment_url:   document.getElementById('lf-attach')?.value || '',
        ...
        try {
            if (this._editingId) {
                await API.cms.updateLesson(this._editingId, p);
                this._toast('Lesson updated');
            } else {
                await API.cms.createLesson(p);
                this._toast('Lesson added');
            }
            this._lmsCloseOverlay();
            await this._lmsLoadLessons(mid);
        } catch (e) {
            this._toast(e.message, 'error');
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-save"></i> Save'; }
        }
    ...
    async _lmsDeleteLesson(id, mid) {
        if (!confirm('Delete this lesson?')) return;
        try {
            await API.cms.deleteLesson(id);
            this._toast('Lesson deleted');
            await this._lmsLoadLessons(mid);
        } catch (e) { this._toast(e.message, 'error'); }
    },
    ...
    async _lmsShowQuizPanel(mid) {
        this._lmsOverlay(`<div class="text-gray-400 text-sm">
            <i class="fas fa-spinner fa-spin mr-2"></i>Loading quiz…
        </div>`);
        let quiz = null;
        try { quiz = await API.cms.getQuiz(mid); } catch (_) {}
    ...
    _lmsRenderQuizEditor(mid, quiz) {
        ...
        <div class="chdr">
            <h4 class="text-white font-semibold">
                <i class="fas fa-question-circle text-yellow-400 mr-2"></i>Quiz — Module ${mid}
            </h4>
            <button class="cb cb-g" onclick="CMSPage._lmsCloseOverlay()"><i class="fas fa-times"></i></button>
        </div>
        ${!quiz ? `
        <div class="crow crow2">
            <div><label class="cl">Quiz Title *</label>
                <input class="ci" id="qz-title" placeholder="Module Quiz"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;">
                <div><label class="cl">Pass %</label>
                    <input type="number" class="ci" id="qz-pass" value="70" min="0" max="100"></div>
                <div><label class="cl">Max Attempts</label>
                    <input type="number" class="ci" id="qz-attempts" value="3" min="1"></div>
            </div>
        </div>
        <div style="display:flex;gap:.75rem;">
            <button class="cb cb-p" onclick="CMSPage._lmsCreateQuiz(${mid})">
        ...
        <div class="crow crow2">
            <div><label class="cl">Quiz Title</label>
                <input class="ci" id="qz-title" value="${this._e(quiz.title)}"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;">
                <div><label class="cl">Pass %</label>
                    <input type="number" class="ci" id="qz-pass" value="${quiz.pass_percentage || 70}" min="0" max="100"></div>
                <div><label class="cl">Max Attempts</label>
                    <input type="number" class="ci" id="qz-attempts" value="${quiz.max_attempts || 3}" min="1"></div>
            </div>
        </div>
        <div style="display:flex;gap:.5rem;margin-bottom:1rem;flex-wrap:wrap;">
            <button class="cb cb-p" onclick="CMSPage._lmsUpdateQuiz(${quiz.id}, ${mid})">
                <i class="fas fa-save"></i> Save Settings
            </button>
            <button class="cb cb-r" onclick="CMSPage._lmsDeleteQuiz(${quiz.id}, ${mid})">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem;">
            ...
            <button class="cb cb-p" style="font-size:.75rem;"
                    onclick="CMSPage._lmsShowQuestionForm(${quiz.id}, ${mid})">
                <i class="fas fa-plus"></i> Add Question
            </button>
        </div>
        <div id="quiz-questions-list">
            ${(quiz.questions || []).map((q,i) => this._lmsQuestionHtml(q, i, mid)).join('')}
        </div>`}`;
    },
    ...
    _lmsQuestionHtml(q, i, mid) {
        ...
        return `<div class="quiz-q">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem;margin-bottom:.5rem;">
                ...
                <div style="display:flex;gap:.3rem;flex-shrink:0;">
                    <button class="cb cb-g" style="font-size:.68rem;padding:.2rem .5rem;"
                            onclick='CMSPage._lmsShowQuestionForm(${q.quiz_id||0}, ${mid}, ${JSON.stringify(q).replace(/'/g,"&#39;")})'>
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="cb cb-r" style="font-size:.68rem;padding:.2rem .5rem;"
                            onclick="CMSPage._lmsDeleteQuestion(${q.id}, ${mid})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.3rem;">
                ...
                <div style="background:${q.correct_option===o?'rgba(16,185,129,.15)':'rgba(31,41,55,.8)'};
                            border:1px solid ${q.correct_option===o?'rgba(16,185,129,.4)':'#374151'};
                            border-radius:.4rem;padding:.35rem .6rem;font-size:.78rem;
                            color:${q.correct_option===o?'#34d399':'#9ca3af'};">
                    <span style="font-weight:700;margin-right:.35rem;">${o.toUpperCase()}.</span>
                    ${this._e(q[`option_${o}`])}
                    ${q.correct_option===o?'<i class="fas fa-check ml-1"></i>':''}
                </div>`).join('')}
            </div>
            ${q.explanation ? `<div style="margin-top:.4rem;font-size:.75rem;color:#6b7280;">
                <i class="fas fa-info-circle mr-1"></i>${this._e(q.explanation)}
            </div>` : ''}
        </div>`;
    },

    async _lmsCreateQuiz(mid) {
        const t = document.getElementById('qz-title')?.value.trim();
        if (!t) { this._toast('Quiz title required', 'error'); return; }
        try {
            await API.cms.createQuiz({
                module_id: mid, title: t,
                pass_percentage: parseInt(document.getElementById('qz-pass')?.value) || 70,
                max_attempts:    parseInt(document.getElementById('qz-attempts')?.value) || 3,
            });
            this._toast('Quiz created');
            await this._lmsShowQuizPanel(mid);
        } catch (e) { this._toast(e.message, 'error'); }
    },

    async _lmsUpdateQuiz(qid, mid) {
        const t = document.getElementById('qz-title')?.value.trim();
        try {
            await API.cms.updateQuiz(qid, {
                module_id: mid, title: t,
                pass_percentage: parseInt(document.getElementById('qz-pass')?.value) || 70,
                max_attempts:    parseInt(document.getElementById('qz-attempts')?.value) || 3,
            });
            this._toast('Quiz settings saved');
        } catch (e) { this._toast(e.message, 'error'); }
    },

    async _lmsDeleteQuiz(qid, mid) {
        if (!confirm('Delete quiz and all questions?')) return;
        try {
            await API.cms.deleteQuiz(qid);
            this._toast('Quiz deleted');
            await this._lmsShowQuizPanel(mid);
        } catch (e) { this._toast(e.message, 'error'); }
    },
    ...
    _lmsShowQuestionForm(quizId, mid, q) {
        const isEdit = q && q.id;
        const d = q || { question:'', option_a:'', option_b:'', option_c:'', option_d:'',
                         correct_option:'a', explanation:'', order_index:0 };
        // Open as a floating modal so it doesn't clobber the quiz panel
        ...
        overlay.innerHTML = `
        <div style="background:#1f2937;border:1px solid #374151;border-radius:.75rem;
                    padding:1.5rem;width:min(620px,95vw);max-height:90vh;overflow-y:auto;">
            <div class="chdr">
                <h4 class="text-white font-semibold">${isEdit ? 'Edit Question' : 'Add Question'}</h4>
                <button class="cb cb-g" onclick="document.getElementById('qform-overlay').remove()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="crow"><div><label class="cl">Question *</label>
                <textarea class="cta" id="qf-q" style="min-height:60px;">${this._e(d.question)}</textarea>
            </div></div>
            <div class="crow crow2">
            ...
            <div class="crow crow2">
                <div><label class="cl">Correct Answer *</label>
                    <select class="cs" id="qf-correct">
                        ${['a','b','c','d'].map(o =>
                            `<option value="${o}"${d.correct_option===o?' selected':''}>${o.toUpperCase()}</option>`).join('')}
                    </select>
                </div>
                <div><label class="cl">Order</label>
                    <input type="number" class="ci" id="qf-ord" value="${d.order_index || 0}"></div>
            </div>
            <div class="crow"><div><label class="cl">Explanation (shown after answer)</label>
                <input class="ci" id="qf-expl" value="${this._e(d.explanation || '')}"></div>
            </div>
            <div style="display:flex;gap:.75rem;margin-top:.5rem;">
                <button class="cb cb-p" onclick="CMSPage._lmsSaveQuestion(${quizId}, ${mid}, ${isEdit ? d.id : 'null'})">
                    <i class="fas fa-save"></i> ${isEdit ? 'Update' : 'Add'} Question
                </button>
    ...
    async _lmsSaveQuestion(quizId, mid, qid) {
        ...
        if (!question || !a || !b) {
            this._toast('Question and options A & B are required', 'error'); return;
        }
        const p = {
            quiz_id:        quizId,
            question,
        ...
        try {
            if (qid) {
                await API.cms.updateQuestion(qid, p);
                this._toast('Question updated');
            } else {
                await API.cms.createQuestion(p);
                this._toast('Question added');
            }
            document.getElementById('qform-overlay')?.remove();
            await this._lmsShowQuizPanel(mid);
        } catch (e) { this._toast(e.message, 'error'); }
    ...
    async _lmsDeleteQuestion(id, mid) {
        if (!confirm('Delete this question?')) return;
        try {
            await API.cms.deleteQuestion(id);
            this._toast('Question deleted');
            await this._lmsShowQuizPanel(mid);
        } catch (e) { this._toast(e.message, 'error'); }
    },
