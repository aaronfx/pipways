...
const DashboardController = class {
    ...
    _patchQuillHtmlPaste() {
        // Quill.find() works on the CONTAINER (.ql-container), not the editor (.ql-editor)
        const containerEl = document.querySelector('#cms-container .ql-container');
        ...
        // Resolve the Quill instance — try all known methods
        // Must use containerEl (not editorEl) for Quill.find() to work
        if (!window.quill) {
            if (window._quillInstance) {
                window.quill = window._quillInstance;
            } else if (typeof Quill !== 'undefined' && typeof Quill.find === 'function') {
                window.quill = Quill.find(containerEl);
            }
        }

        if (window.quill) {
            console.log('[CMS] Quill instance found and exposed as window.quill ✓');
        } else {
            console.warn('[CMS] Quill instance not found via Quill.find() — paste will use DOM fallback');
        }
