...
const courses = {
    async loadCourses() {
        ...
        container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">Loading courses...</p></div>';
        ...
        try {
            ...
            if (!data || !Array.isArray(data.courses)) {
                container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">No courses available</p></div>';
                return;
            }
            ...
            if (coursesList.length === 0) {
                container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">No courses found</p></div>';
                return;
            }
            ...
            container.innerHTML = `<div class="content-card"><p style="text-align: center; color: var(--danger); padding: 40px;">Failed to load courses: ${error.message}</p></div>`;
        }
    },
    ...
    async loadCourseDetail(courseId) {
        if (!auth.currentUser) {
            ui.showToast('Please login to view course details', 'error');
            return;
        }
