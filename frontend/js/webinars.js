...
const webinars = {
    async loadWebinars() {
        ...
        container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">Loading webinars...</p></div>';

        try {
            const data = await api.get('/api/webinars?upcoming=true');
            
            if (!data || !Array.isArray(data.webinars)) {
                container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">No upcoming webinars scheduled</p></div>';
                return;
            }
            ...
            container.innerHTML = `<div class="content-card"><p style="text-align: center; color: var(--danger); padding: 40px;">Failed to load webinars: ${error.message}</p></div>`;
        }
    },
            ...
            <div class="content-card webinar-card" style="${webinar.is_premium ? 'border: 2px solid var(--warning);' : ''}">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    ${webinar.meeting_link && !webinar.is_premium ? 
                        ...
                        webinar.is_premium ? '<button class="btn btn-premium" onclick="ui.showToast(\'VIP subscription required\', \'warning\')">VIP Login Required</button>' : ''}
                </div>
            </div>
        `;
    }
