...
const blog = {
    async loadBlogPosts() {
        ...
        try {
            ...
            if (!data || !Array.isArray(data.posts)) {
                container.innerHTML = '<div class="content-card"><p style="text-align: center; color: var(--text-secondary); padding: 40px;">No articles found</p></div>';
                return;
            }
            ...
            container.innerHTML = `<div class="content-card"><p style="text-align: center; color: var(--danger); padding: 40px;">Error: ${error.message}</p></div>`;
        }
    },
    ...
    async loadPostDetail(postId) {
        if (!auth.currentUser) {
            ui.showToast('Please login to read articles', 'error');
            return;
        }
