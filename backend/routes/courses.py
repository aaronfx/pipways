...
@router.get("/")
async def get_courses(status: Optional[str] = "published", category: Optional[str] = None):
    """Get courses list"""
...
async def get_course(course_id: int, current_user: Optional[dict] = Depends(get_current_user_optional)):
    ...
    async with database.db_pool.acquire() as conn:
        ...
        lessons = await conn.fetch("SELECT * FROM lessons WHERE course_id = $1 ORDER BY order_index ASC", course_id)
        ...
        if current_user:
            ...
            course_dict['progress'] = enrollment['progress_percent'] if enrollment else 0
        else:
...
async def create_course(course: CourseCreate, current_user: dict = Depends(get_admin_user)):
    ...
    async with database.db_pool.acquire() as conn:
        course_id = await conn.fetchval("""
            INSERT INTO courses (title, description, short_description, category, level,
                duration_minutes, instructor_id, thumbnail_url, status, created_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP)
            RETURNING id
        """, course.title, course.description, course.short_description, course.category,
            course.level, course.duration_minutes, current_user['id'], course.thumbnail_url, course.status or 'draft')
        
        return await conn.fetchrow("SELECT * FROM courses WHERE id = $1", course_id)

@router.put("/{course_id}")
async def update_course(course_id: int, course_update: CourseUpdate, current_user: dict = Depends(get_admin_user)):
    ...
    async with database.db_pool.acquire() as conn:
        ...
        for field in ['title', 'description', 'short_description', 'category', 'level', 
                     'duration_minutes', 'thumbnail_url', 'status']:
...
async def create_lesson(course_id: int, lesson: LessonCreate, current_user: dict = Depends(get_admin_user)):
    ...
    async with database.db_pool.acquire() as conn:
        lesson_id = await conn.fetchval("""
            INSERT INTO lessons (course_id, title, content, video_url, order_index, duration_minutes, is_preview, created_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP)
            RETURNING id
        """, course_id, lesson.title, lesson.content, lesson.video_url, 
            lesson.order_index, lesson.duration_minutes, lesson.is_preview or False)
        
        return await conn.fetchrow("SELECT * FROM lessons WHERE id = $1", lesson_id)

@router.post("/{course_id}/questions")
async def ask_question(course_id: int, question: QuestionCreate, current_user: dict = Depends(get_current_user)):
    ...
    async with database.db_pool.acquire() as conn:
        question_id = await conn.fetchval("""
            INSERT INTO student_questions (course_id, lesson_id, user_id, question, is_answered, created_at)
            VALUES ($1, $2, $3, $4, FALSE, CURRENT_TIMESTAMP)
            RETURNING id
        """, course_id, question.lesson_id, current_user['id'], question.question)
