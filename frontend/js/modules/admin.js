/**
 * AdminPage — Professional Admin Dashboard
 *
 * Four sections:
 *   1. Overview      — KPI cards + user growth chart + recent users table
 *   2. User Mgmt     — paginated searchable user table with activate/deactivate
 *   3. Trading       — win-rate donut, top symbols bar chart, signal analytics
 *   4. AI Monitor    — daily AI usage chart + feature cards + top mentor topics
 *
 * Integration:
 *   Call  AdminPage.render(container)  from loadAdminData() in DashboardController.
 *   Requires: Chart.js (already loaded in dashboard.html), API (api.js).
 *
 * Bugs fixed:
 *   - stats.total_courses / recent_users were undefined  → all fields now safe
 *   - UI.escapeHtml / UI.formatDate not guaranteed       → inline helpers used
 *   - No charts, no user management, no AI monitor       → all built here
 *   - AdminPage.render() ignored container arg           → fixed
 */
...
const AdminPage = {
    ...
    _usersPage:  1,
    _usersSearch: '',
    _container:  null,  // root container passed to render()
    ...
    async render(container) {
        if (!container) {
            console.error('[AdminPage] render() called without a container');
            return;
        }
        this._container = container;

        // Check admin access on the client side before even hitting the API
        const user = (() => {
        ...
        // Load all data in parallel; each section is resilient to errors
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // OUTER SHELL  (tabs + section containers)
    // ─────────────────────────────────────────────────────────────────────────
    _shell() {
        return `
        <!-- ── Admin header ── -->
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="AdminPage._refresh()"
                        class="px-3 py-1.5 rounded-lg text-xs font-medium text-gray-400 hover:text-white bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-all">
        ...
        <!-- ── Tabs ── -->
        <div class="flex gap-1 mb-6 p-1 rounded-xl bg-gray-800/60 border border-gray-700 w-fit">
        ...
        <style>
            .admin-tab {
                padding:.45rem 1rem; border-radius:.6rem; font-size:.82rem;
                font-weight:600; border:none; cursor:pointer; color:#9ca3af;
                background:transparent; transition:all .18s; white-space:nowrap;
            }
            .admin-tab.active {
                background:linear-gradient(135deg,#7c3aed,#6d28d9);
                color:white; box-shadow:0 4px 12px rgba(124,58,237,.35);
            }
            ...
            .kpi-card { background:#1f2937; border:1px solid #374151; border-radius:.75rem;
                padding:1.25rem; transition:transform .2s,box-shadow .2s; position:relative; overflow:hidden; }
            ...
            .kpi-card.purple::before { background:linear-gradient(90deg,#7c3aed,transparent); }
            ...
            .kpi-card.orange::before { background:linear-gradient(90deg,#f59e0b,transparent); }
            .kpi-card.red::before    { background:linear-gradient(90deg,#ef4444,transparent); }
            .kpi-card.pink::before   { background:linear-gradient(90deg,#ec4899,transparent); }

            /* User table */
            .ua-table { width:100%; border-collapse:collapse; }
            .ua-table th { padding:.6rem .85rem; text-align:left; font-size:.72rem;
    ...
    _renderOverview(s) {
        ...
        const kpis = [
            {
                color: 'purple', icon: 'fa-users', label: 'Total Users',
                val: this._num(s.total_users), sub: `+${this._num(s.new_today)} today`,
            },
            {
                color: 'green', icon: 'fa-satellite-dish', label: 'Active Signals',
                val: this._num(s.active_signals), sub: `${this._num(s.total_signals)} all-time`,
            },
            {
                color: 'blue', icon: 'fa-graduation-cap', label: 'Courses',
                val: this._num(s.total_courses), sub: 'available',
            },
            {
                color: 'orange', icon: 'fa-video', label: 'Upcoming Webinars',
                val: this._num(s.upcoming_webinars), sub: 'scheduled',
            },
            {
                color: 'pink', icon: 'fa-robot', label: 'AI Mentor Requests',
                val: this._num(s.ai_mentor_requests_today), sub: 'today',
            },
            {
                color: 'red', icon: 'fa-chart-bar', label: 'Charts Analyzed',
                val: this._num(s.charts_analyzed_today), sub: 'today',
            },
            {
                color: 'green', icon: 'fa-file-upload', label: 'Journal Uploads',
                val: this._num(s.journal_uploads_today), sub: 'today',
            },
            {
                color: 'blue', icon: 'fa-newspaper', label: 'Blog Posts',
                val: this._num(s.total_blog_posts), sub: 'published',
            },
        ];
                    ...
                    <div class="kpi-card ${k.color}">
                        <div class="flex items-center justify-between mb-3">
                            <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                                ...
                                <i class="fas ${k.icon} text-sm" style="color:var(--kpi-${k.color},#a78bfa);"></i>
                            </div>
                        </div>
                        ...
                        <p class="text-xs mt-1" style="color:#4b5563;">${k.sub}</p>
                    </div>`).join('')}
            </div>
        ...
        if (growthCtx) {
            const growth = (s.user_growth || []);
            const labels = growth.length
                ? growth.map(g => g.date ? new Date(g.date).toLocaleDateString('en-GB', { day:'2-digit', month:'short' }) : '')
                : ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
            const data   = growth.length
                ? growth.map(g => this._num(g.count))
                : [0,0,0,0,0,0,0];

            this._charts['admin-growth-chart'] = new Chart(growthCtx, {
                type: 'bar',
                data: {
                    labels,
                    datasets: [{
                ...
                options: {
                    ...
                    scales: {
                        y: { grid: { color: 'rgba(75,85,99,.3)' }, ticks: { color: '#9ca3af', stepSize: 1 }, beginAtZero: true },
        ...
        // Platform activity doughnut
        this._destroyChart('admin-activity-chart');
        const actCtx = document.getElementById('admin-activity-chart');
        if (actCtx) {
            this._charts['admin-activity-chart'] = new Chart(actCtx, {
                type: 'doughnut',
                data: {
                    labels: ['AI Mentor', 'Chart Analysis', 'Journal Uploads', 'Signals'],
                    datasets: [{
                        data: [
    ...
    async _renderUsersSection() {
        ...
        el.innerHTML = `
            <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-700">
                    <h4 class="font-semibold text-white text-sm">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <div class="relative flex-1 sm:flex-none">
                            ...
                            <input type="text" id="admin-user-search"
                                   ...
                                   class="w-full sm:w-48 pl-8 pr-3 py-2 text-sm bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-600 focus:outline-none focus:border-purple-500"
                                   oninput="AdminPage._onSearchInput(this.value)">
                        </div>
                    </div>
                </div>
                <div id="admin-users-body" class="overflow-x-auto"></div>
                <div id="admin-users-pagination" class="px-5 py-4 border-t border-gray-700 flex items-center justify-between"></div>
            </div>`;
    ...
    _onSearchInput: (() => {
        ...
        return (val) => {
            clearTimeout(timer);
            timer = setTimeout(() => AdminPage._loadUsersPage(1, val), 350);
        };
    })(),
    ...
    async _loadUsersPage(page, search) {
        ...
        const pager = document.getElementById('admin-users-pagination');
        if (!body) return;
        ...
        try {
            ...
            body.innerHTML = `
                <table class="ua-table">
                    ...
                    <tbody>
                        ${users.map(u => this._userManagementRow(u)).join('')}
                    </tbody>
                </table>`;
                ...
                pager.innerHTML = `
                    ...
                    <div class="flex gap-1">
                        <button onclick="AdminPage._loadUsersPage(${page-1},'${this._esc(search)}')"
                                ${page <= 1 ? 'disabled' : ''}
                                class="px-3 py-1.5 rounded-lg text-xs bg-gray-700 text-gray-300 disabled:opacity-30 hover:bg-gray-600 transition-colors">
                            ← Prev
                        </button>
    ...
    _userRow(u) {
        ...
        const tierColor = tier === 'pro' ? '#a78bfa' : tier === 'enterprise' ? '#fbbf24' : '#6b7280';
        return `
            <tr>
                ...
                <td><span style="color:${tierColor};font-size:.75rem;font-weight:600;">${this._esc(tier)}</span></td>
                <td>
                    <span class="ua-badge" style="${active ? 'background:rgba(16,185,129,.15);color:#34d399;border:1px solid rgba(16,185,129,.3);' : 'background:rgba(239,68,68,.15);color:#f87171;border:1px solid rgba(239,68,68,.3);'}">
    ...
    _userManagementRow(u) {
        const active  = u.is_active !== false;
        const isAdmin = u.is_admin === true;
        const tier    = u.subscription_tier || 'free';
        const tierColors = { pro: '#a78bfa', enterprise: '#fbbf24', free: '#6b7280' };
        ...
        return `
            <tr>
                ...
                <td class="text-gray-400">${this._esc(u.email)}</td>
                <td class="text-gray-500">${this._date(u.created_at)}</td>
                <td><span style="color:${tc};font-size:.72rem;font-weight:700;">${this._esc(tier.toUpperCase())}</span></td>
                <td>
                    ${isAdmin
                        ? '<span class="ua-badge" style="background:rgba(239,68,68,.15);color:#f87171;border:1px solid rgba(239,68,68,.3);">Admin</span>'
                        : '<span class="ua-badge" style="background:rgba(107,114,128,.15);color:#9ca3af;border:1px solid rgba(107,114,128,.3);">User</span>'
                    }
                </td>
                <td>
                    <span class="ua-badge" style="${active ? 'background:rgba(16,185,129,.15);color:#34d399;border:1px solid rgba(16,185,129,.3);' : 'background:rgba(239,68,68,.15);color:#f87171;border:1px solid rgba(239,68,68,.3);'}">
    ...
    async _toggleUser(userId, btn) {
        ...
        try {
            ...
            // Brief toast
            this._toast(result.message || 'User status updated', 'success');
        } catch (err) {
            btn.textContent = original;
            btn.disabled = false;
            this._toast(err.message || 'Failed to update user', 'error');
        }
    },
    ...
    _renderTradingSection(t) {
        ...
        const wr        = this._num(t.overall_win_rate, 0);
        const total     = this._num(t.total_signals);
        const wins      = this._num(t.wins);
        const losses    = this._num(t.losses);
        const pending   = this._num(t.pending);
        const buyCount  = this._num(t.buy_count);
        const sellCount = this._num(t.sell_count);
        const avgConf   = this._num(t.avg_ai_confidence, 0);
        const symbols   = t.top_symbols || [];
        const monthly   = t.monthly_signals || [];

        el.innerHTML = `
            <!-- Win-rate headline row -->
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ${[
                    { color:'green',  icon:'fa-trophy',         label:'Win Rate',        val:`${wr}%`          },
                    { color:'blue',   icon:'fa-satellite-dish',  label:'Total Signals',   val:total             },
                    { color:'purple', icon:'fa-robot',           label:'Avg AI Confidence', val:`${avgConf}%`   },
                    { color:'orange', icon:'fa-balance-scale',   label:'BUY vs SELL',     val:`${buyCount}/${sellCount}` },
                ].map(k => `
                    <div class="kpi-card ${k.color}">
                        <i class="fas ${k.icon} text-sm mb-3 block" style="color:var(--kpi-${k.color},#a78bfa);opacity:.7;"></i>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                ...
                <div class="bg-gray-800 rounded-xl p-5 border border-gray-700">
                    ...
                    <div style="height:240px;position:relative;">
                        <canvas id="admin-outcome-chart"></canvas>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-xl p-5 border border-gray-700">
                    <h4 class="font-semibold text-white text-sm mb-4">
                        <i class="fas fa-chart-bar text-blue-400 mr-2"></i>Monthly Signal Volume
                    </h4>
                    <div style="height:240px;position:relative;">
                        <canvas id="admin-monthly-chart"></canvas>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="ua-table">
                        ...
                        <tbody>
                            ${symbols.length > 0
                                ? symbols.map((s, i) => {
                                    const wr2 = this._num(s.win_rate, 0);
                                    const col = wr2 >= 60 ? '#34d399' : wr2 >= 45 ? '#fbbf24' : '#f87171';
                                    return `
                                    <tr>
                                        ...
                                        <td style="color:${col};font-weight:700;">${wr2}%</td>
                                        <td>
                                            <div style="width:120px;height:6px;background:#1f2937;border-radius:3px;overflow:hidden;">
                                                <div style="width:${wr2}%;height:100%;background:${col};border-radius:3px;"></div>
                                            </div>
                                        </td>
                                    </tr>`;
                                  }).join('')
        ...
        const outcomeCtx = document.getElementById('admin-outcome-chart');
        if (outcomeCtx) {
            this._charts['admin-outcome-chart'] = new Chart(outcomeCtx, {
                type: 'doughnut',
                data: {
                    labels: ['Wins', 'Losses', 'Pending'],
                    datasets: [{
                        data: [wins || 1, losses || 1, pending || 1],
        ...
        const monthlyCtx = document.getElementById('admin-monthly-chart');
        if (monthlyCtx) {
            const fallback = ['Jan','Feb','Mar','Apr','May','Jun'];
            this._charts['admin-monthly-chart'] = new Chart(monthlyCtx, {
                type: 'bar',
                data: {
                    labels: monthly.length ? monthly.map(m => m.month) : fallback,
                    datasets: [{
                        label: 'Signals',
                ...
                options: {
                    ...
                    plugins: { legend: { display: false } },
                    scales: {
                        y: { grid: { color:'rgba(75,85,99,.3)' }, ticks: { color:'#9ca3af' }, beginAtZero: true },
                        x: { grid: { display: false }, ticks: { color:'#9ca3af' } },
                    },
                },
            });
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 4 — AI MONITOR
    // ─────────────────────────────────────────────────────────────────────────
    _renderAISection(a) {
        const el = document.getElementById('admin-tab-ai');
        if (!el) return;

        const mentorToday  = this._num(a.mentor_requests_today);
        const mentorWeek   = this._num(a.mentor_requests_week);
        const chartsToday  = this._num(a.charts_analyzed_today);
        const chartsWeek   = this._num(a.charts_analyzed_week);
        const jToday       = this._num(a.journal_uploads_today);
        const jMonth       = this._num(a.journal_uploads_month);
        const dailyUsage   = a.daily_ai_usage   || [];
        const topTopics    = a.top_mentor_topics || [];
        ...
        el.innerHTML = `
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ${[
                    {
                        icon:'fa-robot', color:'#a78bfa', bg:'rgba(124,58,237,.15)',
                        label:'AI Mentor',
                        today: mentorToday, week: mentorWeek, weekLabel:'this week'
                    },
                    {
                        icon:'fa-chart-bar', color:'#60a5fa', bg:'rgba(59,130,246,.15)',
                        label:'Chart Analysis',
                        today: chartsToday, week: chartsWeek, weekLabel:'this week'
                    },
                    {
                        icon:'fa-file-upload', color:'#34d399', bg:'rgba(16,185,129,.15)',
                        label:'Journal Uploads',
                        today: jToday, week: jMonth, weekLabel:'this month'
                    },
                ].map(c => `
                    <div class="bg-gray-800 rounded-xl p-5 border border-gray-700">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-700 flex items-center justify-between text-xs">
                            <span class="text-gray-500">${c.weekLabel}</span>
                            <span style="color:${c.color};font-weight:700;">${c.week}</span>
                        </div>
                    </div>`).join('')}
            </div>

            <!-- Daily AI usage chart + top topics -->
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div class="lg:col-span-2 bg-gray-800 rounded-xl p-5 border border-gray-700">
                    ...
                    <div style="height:240px;position:relative;">
                        <canvas id="admin-ai-usage-chart"></canvas>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-xl p-5 border border-gray-700">
                    ...
                    ${topTopics.length > 0
                        ? `<div class="space-y-3">
                            ${topTopics.map((t, i) => {
                                const maxCount = Math.max(...topTopics.map(x => this._num(x.count)), 1);
                                const pct = Math.round(this._num(t.count) / maxCount * 100);
                        ...
                        : `<div class="flex flex-col items-center justify-center h-40 text-gray-600 text-sm">
                               <i class="fas fa-comment-slash text-2xl mb-2 opacity-30"></i>
                               No topic data yet
                           </div>`
                    }
                </div>
        ...
        const usageCtx = document.getElementById('admin-ai-usage-chart');
        if (usageCtx) {
            const fallbackDays = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
            const labels = dailyUsage.length
                ? dailyUsage.map(d => d.date
                    ? new Date(d.date).toLocaleDateString('en-GB', { weekday:'short' })
                    : '')
                : fallbackDays;
            const data = dailyUsage.length
                ? dailyUsage.map(d => this._num(d.requests))
                : [0,0,0,0,0,0,0];

            this._charts['admin-ai-usage-chart'] = new Chart(usageCtx, {
                type: 'line',
                data: {
                    labels,
                    datasets: [{
    ...
    _toast(message, type = 'info') {
        ...
        div.style.cssText = `
            ...
            font-weight:600; color:white; max-width:320px;
