...
async def lifespan(app: FastAPI):
    try:
        ...
        # ── Initialise Stock Terminal clients ──────────────────────────────
        # Now correctly targets the same module instance the router runs in.
        try:
            anthropic_key = os.environ.get("ANTHROPIC_API_KEY", "")
            if anthropic_key:
                stock_module._anthropic = AsyncAnthropic(api_key=anthropic_key)
                stock_module._http      = httpx.AsyncClient(timeout=10.0)
                print("[STOCK] Terminal clients initialised", flush=True)
            else:
...
async def health_check():
    return {
        ...
        "stock_terminal": {
            ...
            "http_ready":      stock_module._http is not None,
        },
        "features": [
