...
async def init_database():
    """Alias for init_db — called by main.py lifespan startup."""
    await database.connect()

async def run_migrations():
    """
    Alias for the migration already run inside database.connect().
    main.py calls this explicitly after init_database(); it is safe to call
    again because all DDL uses IF NOT EXISTS / IF NOT EXISTS guards.
    """
    await database.run_migrations()

async def run_unique_index_migrations():
    """
    Placeholder for unique-index migrations.
    Extend this if you add unique indexes that need separate DDL.
